...
pub fn parse_backtrace_line(line: &str) -> ParseResult<BacktraceFrame> {
    ...
    parse_frame(input).map(|(_, frame)| frame).map_err(|e| {
        ParseError::InvalidBacktrace(format!("Failed to parse frame: {}", e))
    })
}
...
fn parse_frame(input: &str) -> IResult<&str, BacktraceFrame> {
    ...
    if rest.starts_with('(') {
        if let Ok((rest2, (func, offset))) = parse_function_info(rest) {
